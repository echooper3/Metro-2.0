...
// Lazy load heavy components to improve initial bundle size and load speed
...
const App: React.FC = () => {
  ...
  const loadCityEvents = useCallback(async (cityName: string | 'All', options: FetchOptions = {}) => {
    ...
    const hasExistingData = filteredEvents.length > 0;
        ...
        startTransition(() => {
          if (isNextPage) {
            setAllEvents(prev => {
              ...
              const newItems = data.events.filter(n => !existingIds.has(n.id));
  ...
  // Pre-fetching logic for instant category switches
  ...
  const displayedDescription = detailedEvent 
    ? (isDescExpanded || !shouldTruncateDesc ? detailedEvent.description : `${detailedEvent.description.slice(0, 250)}...`) 
    : '';
              ...
              <div className="flex flex-wrap items-center gap-6 mt-12">
                {isRefreshing ? (
                  ...
                  <div className="flex items-center gap-4 border border-white/10 bg-white/5 px-8 py-4 rounded-3xl">
                    ...
                    <span className="font-black text-[11px] uppercase tracking-[0.2em] text-emerald-400">
                      {sourceStatus === 'cache' ? 'Displaying Cached Feed' : 'Live Sync Complete'}
                    </span>
                  </div>
                )}
              </div>
            ...
            <div className="flex overflow-x-auto scrollbar-hide space-x-4 bg-white p-4 rounded-[2rem] shadow-2xl shadow-gray-200 border border-gray-100">
              {CATEGORIES.map(cat => (
                <button 
                  ...
                  className={`px-8 py-4 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] transition-all shrink-0 ${activeCategory === cat ? 'bg-orange-600 text-white shadow-xl shadow-orange-200' : 'bg-gray-50 text-gray-500 hover:bg-orange-50 hover:text-orange-600'}`}
                >
                  {cat}
                </button>
              ))}
              ...
              {isPageLoading && (
                <div className="flex flex-col items-center gap-6">
                   <div className="flex space-x-2">
                     ...
                     <div className="w-3 h-3 bg-orange-600 rounded-full animate-bounce [animation-delay:-0.15s]"></div>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-[3rem] w-full max-w-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 flex flex-col max-h-[90vh]">
            ...
            <div id="event-detail-scroll" className="p-16 pt-12 overflow-y-auto">
              ...
              <div className="mb-12">
                <p className="text-gray-600 text-xl leading-relaxed font-medium mb-4 whitespace-pre-wrap">
                  {displayedDescription}
                </p>
                {shouldTruncateDesc && (
                  <button 
                    onClick={() => setIsDescExpanded(!isDescExpanded)}
                    className="text-orange-600 font-black uppercase tracking-widest text-[10px] hover:underline mb-12 flex items-center gap-2"
                  >
                    {isDescExpanded ? 'Read Less' : 'Read More'}
                    <svg className={`w-3 h-3 transition-transform ${isDescExpanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="mb-16">
                ...
                {relatedEvents.length > 0 && (
                  <div className="pt-12 border-t border-gray-100">
                    <div className="mb-8">
                      <span className="text-orange-600 font-black uppercase tracking-[0.2em] text-[10px] mb-2 block">Recommendations</span>
                      <h3 className="text-3xl font-black text-gray-900 tracking-tighter">Related Experiences</h3>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      {relatedEvents.map(e => (
                        <div key={e.id} className="cursor-pointer" onClick={() => handleOpenDetails(e)}>
                          <div className="bg-gray-50 hover:bg-white border border-transparent hover:border-orange-200 rounded-2xl p-4 transition-all flex gap-4 group">
                            <div className="w-20 h-20 rounded-xl overflow-hidden shrink-0">
                              <img src={e.imageUrl || `https://images.unsplash.com/photo-1501281668745-f7f57925c3b4?auto=format&fit=crop&q=80&w=200`} alt={e.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform" />
                            </div>
                            <div className="flex flex-col justify-center min-w-0">
                              <p className="text-[10px] font-black uppercase tracking-widest text-orange-500 mb-1">{e.category}</p>
                              <h4 className="text-sm font-black text-gray-900 line-clamp-1 group-hover:text-orange-600 transition-colors">{e.title}</h4>
                              <p className="text-[10px] font-bold text-gray-400 mt-1">{e.cityName} • {e.date}</p>
                            </div>
                          </div>
                        </div>
                      ))}
